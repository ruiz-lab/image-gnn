...
from models.models import E2EWanGNN
...
train_data = torch.load('data/WANGraph/wan_train_knn_graph.pkl')
test_data = torch.load('data/WANGraph/wan_test_knn_graph.pkl')

train_data_hyper_nodes = np.array(train_data.x)
test_data_hyper_nodes = np.array(test_data.x)
...
def train(model, loader, use_edges):
    ...
    for batch in tqdm(loader, desc="Train", leave=False):
        ...
        batch.x = train_data_hyper_nodes[batch.n_id]
...
def evaluate(model, loader, use_edges):
    ...
    total_loss, correct, total = 0, 0, 0
    for batch in loader:
        batch.x = test_data_hyper_nodes[batch.n_id]
        batch = batch.to(device)
        out = model(batch, device=device)
...
for sz in sizes:
    ...
    model = E2EWanGNN(1, 32, 64, 'sage', 'sum', 0, 64, 2, 'sage', 21).to(device)
...
#     # model = GNN(train_data.num_node_features, hidden_channels=128, out_channels=num_classes)
#     model = E2EWanGNN(1, 32, 64, 'gcn', 'sum', 0, 64, 2, 'gcn', 21)
#     optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
...
# size_schedule = [375, 750, 1500, 3000, 6000, 12000]
